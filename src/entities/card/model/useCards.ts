...
export const cardKeys = {
  ...
  topics: {
    ...
    list: (params?: { skip?: number; limit?: number }) => [...cardKeys.topics.lists(), params] as const,
...
export const useCreateCardTopic = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: cardKeys.topics.lists() });
    },
  });
};
...
export const useUpdateCardTopic = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: cardKeys.topics.detail(variables.id) });
      queryClient.invalidateQueries({ queryKey: cardKeys.topics.lists() });
    },
  });
};
