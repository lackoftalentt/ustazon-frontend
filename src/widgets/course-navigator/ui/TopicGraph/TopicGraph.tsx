...
export const TopicGraph: React.FC<TopicGraphProps> = ({ subjectCode, topics = [] }) => {
  const svgRef = useRef<SVGSVGElement>(null);
  const navigate = useNavigate();
  const [dimensions, setDimensions] = useState({ width: 0, height: 0 });
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set(['center']));
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [selectedParentTopic, setSelectedParentTopic] = useState<{ id: number; name: string } | null>(null);
  ...
  const graphData = useMemo((): GraphData => {
    ...
    const addTopicWithChildren = (topic: CardTopic, parentId: string, depth: number = 0) => {
    ...
    if (topics.length > 0) {
      ...
      const topLevelTopics = topics.filter(t => t.parent_topic_id === null);
      ...
      setExpandedNodes(prev => {
        ...
        if (next.has(node.id)) {
          ...
          const collapseRecursive = (id: string) => {
            next.delete(id);
            graphData.nodes.filter(n => n.parentId === id).forEach(n => collapseRecursive(n.id));
          };
