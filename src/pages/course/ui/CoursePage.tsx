...
import { TopicGraph } from '@/widgets/course-navigator';
...
export const CoursePage = () => {
    ...
    // Get selected topic ID and window ID from URL query params
    ...
    // Fetch cards for this subject with optional window filter
    const { data: cards, isLoading: isLoadingCards } = useCards(
        subject?.id
            ? {
                subject_id: subject.id,
                window_id: selectedWindowId ? parseInt(selectedWindowId) : undefined,
                limit: 100000,
              }
            : undefined
    );

    // Fetch ALL topics with hierarchy from API (we'll filter by subject later)
    const { data: allTopics, isLoading: isLoadingTopics } = useCardTopics({ limit: 1000 });
        ...
        cards.forEach(card => {
            ...
            if (grade !== null && grade !== undefined) {
                ...
                if (quarter !== null && quarter !== undefined) {
                    ...
                    if (topic) {
                        ...
                        if (!quarterNode.children.find((t: any) => t.id === topic.id)) {
                            quarterNode.children.push({ ...topic, children: topic.children || [] });
                        }
                    }
                } else if (topic) {
                    ...
                    if (!gradeNode.children.find((t: any) => t.id === topic.id)) {
                        gradeNode.children.push({ ...topic, children: topic.children || [] });
                    }
                }
            } else if (topic) {
                ...
                if (!gradeQuarterTopics.has(topicKey)) {
                    gradeQuarterTopics.set(topicKey, { ...topic, children: topic.children || [] });
                }
            }
        });
    ...
    const filteredCards = useMemo(() => {
        ...
        if (selectedTopicId) {
            filtered = filtered.filter(card => card.topic?.id === parseInt(selectedTopicId));
        }
    ...
    const selectedTopic = topics.find(t => t.id === parseInt(selectedTopicId || '0'));
            ...
            <Container>
                ...
                {selectedTopicId && selectedTopic && (
                    <div style={{
                        ...
                        <Link
                            to={`/course/${subjectCode}`}
                            style={{ color: '#0284c7', textDecoration: 'underline', fontSize: '14px' }}
                        >
                            Сбросить фильтр
                        </Link>
                    </div>
                ...
                <TopicGraph subjectCode={subjectCode || ''} topics={topics || []} />
                ...
                {selectedWindowId && (
                    <div style={{
                        ...
                        <Link
                            to={`/course/${subjectCode}`}
                            style={{ color: '#2f8450', textDecoration: 'underline', fontSize: '14px' }}
                        >
                            Сбросить фильтр
                        </Link>
                    </div>
                ...
                {/* Window Groups */}
                {!selectedWindowId && cardsByWindow.map(({ window, cards }) => (
                    <div key={window.id}>
                        ...
                        <div className={s.container}>
                            {cards.slice(0, 3).map(card => (
                                <CourseCard
                                    ...
                                    id={card.id}
                                    title={card.name}
                                    description={card.description || `${card.topic?.topic || 'Материал'} • ${card.grade ? `${card.grade} класс` : ''}`}
                                    thumbnail={card.img1_url || undefined}
                                    path={`/course/${subjectCode}/detail/${card.id}`}
                                />
                            ))}
                        </div>
                        {cards.length > 3 && (
                            <Link
                                to={`/course/${subjectCode}?window=${window.id}`}
                                className={s.showMoreLink}
                                style={{ cursor: 'pointer', opacity: 1, color: '#2f8450', fontWeight: 500, textDecoration: 'none', display: 'block', marginTop: '16px' }}
                            >
                ...
                {selectedWindowId && (
                    <div>
                        <SectionTitle title={subject?.windows?.find(w => w.id === parseInt(selectedWindowId))?.name || 'Материалы'} />
                        <div className={s.container}>
                    ...
                    <>
                        ...
                        <div className={s.container}>
                            ...
                            {cardsWithoutWindow.length > 6 && (
                                <div className={s.showMoreLink} style={{ cursor: 'default', opacity: 0.6 }}>
                                    Всего материалов: {cardsWithoutWindow.length}
                                </div>
                            )}
                        </div>
                    </>
                    ...
                    <div style={{ textAlign: 'center', padding: '60px 20px', color: '#888' }}>
                        ...
                        {selectedTopicId && (
                            <div style={{ fontSize: '14px', marginTop: '12px' }}>
                                Попробуйте <Link to={`/course/${subjectCode}`} style={{ color: '#0284c7', textDecoration: 'underline' }}>сбросить фильтр</Link> по теме
                            </div>
                        )}
                    </div>
                )}
                ...
                {filteredCards && filteredCards.length > 0 && (
                    <div style={{
                        ...
                        {selectedTopicId && selectedTopic && ` (тема: ${selectedTopic.topic})`}
                    </div>
                )}
            </Container>
        </main>
